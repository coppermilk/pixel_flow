...
MainWindow::MainWindow(ControllerBoard *pControllerBoard, QWidget *parent)
  ...
  timer->start(1000);
  std::vector<int> seconds_parametes_to_select_in_comboBoxTimer = {15, 60, 900, 1800, 3600};
...
void MainWindow::onColorChanged() {
  ...
  // This is a function that is changing the color in the pixel board.
  colorMax.setRgb(ui.horizontalSliderMaxRedBrightness->value(), 
                  ui.horizontalSliderMaxGreenBrightness->value(),
                  ui.horizontalSliderMaxBlueBrightness->value());
  colorMin.setRgb(ui.horizontalSliderMinRedBrightness->value(), 
                  ui.horizontalSliderMinGreenBrightness->value(),
  ...
  Pixel pixelMin(ui.horizontalSliderMinRedBrightness->value(), 
                 ui.horizontalSliderMinGreenBrightness->value(),
                 ui.horizontalSliderMinBlueBrightness->value());
  Pixel pixelMax(ui.horizontalSliderMaxRedBrightness->value(), 
                 ui.horizontalSliderMaxGreenBrightness->value(),
                 ui.horizontalSliderMaxBlueBrightness->value());
...
void MainWindow::comboBoxActivityChanged(int index) {
  ...
  if (index == ui.comboBoxActivityType->currentIndex()) {
    ...
    onColorChanged();
    BOOST_LOG_TRIVIAL(debug) << "Board change activity to \""
                             << ui.comboBoxActivityType->currentText().toStdString() << "\".";
    return;
  } else {
